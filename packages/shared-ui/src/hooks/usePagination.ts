...
export function usePagination<T>({ pageSize = 9, initialPage = 1, onPageChange, cacheSize = 5, fetchData }: UsePaginationOptions<T> = {}): UsePaginationResult<T> {
    ...
    setState(prev => {
      ...
      if (entries.length >= cacheSize) {
        ...
        const sortedEntries = entries
          .sort((a, b) => Math.abs(prev.currentPage - Number(b[0])) - Math.abs(prev.currentPage - Number(a[0])))
          .slice(0, cacheSize - 1);
        return {
          ...prev,
          cachedData: {
  ...
  const getCachedData = useCallback((page: number) => {
